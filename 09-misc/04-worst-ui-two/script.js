...
(function () {
    ...
    setButtons.addEventListener('click', e => {
        let dataMin = parseInt(e.target.getAttribute("data-min"))
        let dataMax = parseInt(e.target.getAttribute("data-max"))
        var currentNumber = parseInt(e.target.innerText)
        var newNumber = 0;
        var newDial = "";

        if (currentNumber == dataMax) { newNumber = dataMin } else { newNumber = currentNumber + 1 }
